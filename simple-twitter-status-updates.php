...
Plugin URI: http://www.bannerweb.ch/unsere-leistungen/wordpress-dev/bannerweb-tweet-center/
Description: Erm&ouml;glicht das automatische Aktualisieren des eigenen Twitterstatus bei der Ver&ouml;ffentlichung oder &Auml;nderung eines Blogartikels oder wenn ein neuer Kommentar zu einem Artikel geschrieben wird.
Version: 1.0
...
$str_wp_version = preg_replace('#[^0-9]#', '', get_bloginfo('version'));
$str_wp_version = (strlen($str_wp_version) == 3) ? $str_wp_version : $str_wp_version.'0';

if(	phpversion() < 5 or 
	extension_loaded('curl') === false or
	$str_wp_version < 270) {
	
	add_action('admin_notices', 'stsu_compatibility_notice');
	$boo_hide_plugin = true;
}

// Set DEFINE and other plugin settings
else{
	...
	require_once(STSU_PLUGIN_LOCATION.'twitter-async/EpiCurl.php');
	require_once(STSU_PLUGIN_LOCATION.'twitter-async/EpiOAuth.php');
	require_once(STSU_PLUGIN_LOCATION.'twitter-async/EpiTwitter.php');
}

// Display admin incompatibility notice
function stsu_compatibility_notice(){
...
# Run plugin if it is compatible and it is not called directly
# -----------------------------------

if(defined('ABSPATH') and defined('WPINC') and $boo_hide_plugin !== true) {
